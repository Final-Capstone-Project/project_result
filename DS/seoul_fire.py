...
# 사망자 기준 요약
print(df.info())
df_die = df.groupby('자치구').sum()
df_die = df_die[['부상자', '사망자']]
df_die = df_die.sort_values(by = '부상자', ascending= False)
print(df_die.info())

plt.style.use('ggplot')
df_die.plot(kind="bar", color = ['skyblue', 'red'])
plt.xticks(rotation = 45)
plt.ylabel('피해자 수')
plt.title('서울시 화재발생 사망 및 부상 현황 (2017 ~ 2019)')
plt.show()
